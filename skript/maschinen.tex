...
f"ur HPC geeignete Betreibssyteme dem Programmierer auch die
...
kB. Dieser Cache unterscheidet zwischen Daten und Instruktionen.
...
n"utzlichen Teil ihrer Daten und Instruktionen darin finden 
k"onnen. 
...
Aber auch bei anderen bedingten Instruktionen kann es zu sogenannten
...
\end{tabular}
\end{center}
\caption{Bubble in einer Pipeline ausgel"ost durch eine Abh"angigkeit
...
\end{verbatim}
Jetzt ist nur noch bei jedem vierten kopierten Byte mit einem Pipeline-Stall
zu rechnen.
...
$4$ bzw.~\verb+4 * sizeof(long)+ teilbar ist.
...
steuert, spricht man von SIMD: Single Instruction Multiple Data.
\index{SIMD}
...
Code unportabel und schwer wartbar macht.
OpenMP (siehe Abschnitt~\ref{openmp-intro}) unterst"utzt SIMD einheiten
mit den {\tt simd} Direktiven.
