...
\begin{equation}
u(x,y)=g(x,y)\qquad \text{f"ur $x\in\partial \Omega$,}
...
Wir numerieren die verbleibenden Unbekannten neu mit einem
...
(\ref{potenz-iteration}) wiederholt darauf an. Dabei erhalten wir die
Vektoren
\begin{center}
\begin{tabular}{|>{$}r<{$}|>{$}r<{$}>{$}r<{$}>{$}r<{$}|}
\hline
...
\end{align*}
Eine quadratische Funktion der Form $as^2+bs+c$ mit $a>0$ erreicht ihr
Minimum bei $s=-b/2a$. Somit ist die beste Wahl von $s$
\[
